...
export default function VsFriendMatch({ address }: Props) {
    ...
    signAndExecute(
      { transaction: tx },
      {
        onSuccess: (res: any) => {
          ...
          setGameId(id);
          setStage('waiting');
          setStatus(`Game created. Share this code with your friend: ${id}`);
        },
  ...
  async function handleComplete(res: MatchResult) {
    setResult(res);
    setStage('done');
    setStatus('Match complete. Requesting payout from server...');
    const opponentAddress = '0x0'; // placeholder: needs real opponent address tracking
  ...
  if (stage === 'waiting') {
    return (
      <div style={{ maxWidth: 500, margin: '40px auto', color: '#fff', textAlign: 'center' }}>
        ...
        <p style={{ color: '#888' }}>Send them this code. Once they join, refresh and start the match.</p>
